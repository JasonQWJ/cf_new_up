...
function 创建值节点命名器(跳过编号779 = false, 地区前缀 = '') {
  const 计数器组 = {};
  const 前缀 = 地区前缀 ? `${地区前缀} | ` : '';
  return 项目778 => {
    const 基础 = 获取值节点别名基础(项目778);
    if (跳过编号779) return `${前缀}${基础}`;
    计数器组[基础] = (计数器组[基础] || 0) + 1;
    return `${前缀}${基础}-${String(计数器组[基础]).padStart(2, '0')}`;
  };
}
...
async function 处理订阅请求(请求507, 用户506, 网址505 = null) {
  ...
  const url指定的地区 = (网址505.searchParams.get('wk') || '').toUpperCase();
  if (url指定的地区) {
    当前工作器地区 = url指定的地区;
  }
  const 实际订阅地区 = url指定的地区 || 当前工作器地区 || 'CF';
  const 地区名称前缀 = (typeof 地区映射 !== 'undefined' && 地区映射[实际订阅地区] && 地区映射[实际订阅地区][0]) || (实际订阅地区 !== 'CF' ? 实际订阅地区 : '');
  const 别名命名器502 = 创建值节点命名器(false, 地区名称前缀);
  const 附加路径参数 = 实际订阅地区 && 实际订阅地区 !== 'CF' ? `wk=${实际订阅地区}` : '';
  ...
  async function 添加节点列表来源列表(列表498) {
    if (启用明文) {
      最终链接列表.push(...生成链接列表来源源(列表498, 用户506, 工作器域名504, 加密客户端问候配置501, false, 别名命名器502, 附加路径参数));
    }
    if (启用木马) {
      最终链接列表.push(...(await 生成木马链接列表来源源(列表498, 用户506, 工作器域名504, 加密客户端问候配置501, false, 别名命名器502, 附加路径参数)));
    }
    if (启用扩展传输) {
  ...
  if (禁用优选) {} else if (是否有自定义优选) {
    ...
    if (启用仓库优选) {
      try {
        ...
        if (新地址列表.length > 0) {
          if (启用明文) {
            最终链接列表.push(...生成链接列表来源新地址列表(新地址列表, 用户506, 工作器域名504, 加密客户端问候配置501, false, 别名命名器502, 附加路径参数));
          }
          if (启用木马) {
            最终链接列表.push(...(await 生成木马链接列表来源新地址列表(新地址列表, 用户506, 工作器域名504, 加密客户端问候配置501, false, 别名命名器502, 附加路径参数)));
          }
          if (启用扩展传输) {
            最终链接列表.push(...生成扩展超文本链接列表来源源(新地址列表, 用户506, 工作器域名504, 加密客户端问候配置501, false, 别名命名器502, 附加路径参数));
          }
        }
      } catch (错误486) {
        if (!当前工作器地区) {
...
function 生成链接列表来源源(列表482, 用户481, 工作器域名480, 加密客户端问候配置479 = null, 跳过编号478 = false, 别名命名器477 = null, 附加查询 = '') {
  ...
  const 链接列表472 = [];
  const 网页套接字路径471 = '/?ed=2048' + (附加查询 ? `&${附加查询}` : '');
...
async function 生成木马链接列表来源源(列表455, 用户454, 工作器域名453, 加密客户端问候配置452 = null, 跳过编号451 = false, 别名命名器450 = null, 附加查询 = '') {
  const 云墙超文本端口449 = [80, 8080, 8880, 2052, 2082, 2086, 2095];
  const 云墙安全超文本端口448 = [443, 2053, 2083, 2087, 2096, 8443];
  const 默认安全超文本值 = [443];
  const 默认超文本值 = 禁用非传输层安全 ? [] : [80];
  const 链接列表447 = [];
  const 网页套接字路径446 = '/?ed=2048' + (附加查询 ? `&${附加查询}` : '');
...
function 生成链接列表来源新地址列表(列表100, 用户99, 工作器域名98, 加密客户端问候配置97 = null, 跳过编号96 = false, 别名命名器95 = null, 附加查询 = '') {
  const 云墙超文本端口94 = [80, 8080, 8880, 2052, 2082, 2086, 2095];
  const 云墙安全超文本端口93 = [443, 2053, 2083, 2087, 2096, 8443];
  const 链接列表92 = [];
  const 网页套接字路径91 = '/?ed=2048' + (附加查询 ? `&${附加查询}` : '');
...
async function 生成木马链接列表来源新地址列表(列表, 用户, 工作器域名, 加密客户端问候配置 = null, 跳过编号 = false, 别名命名器 = null, 附加查询 = '') {
  const 云墙超文本端口 = [80, 8080, 8880, 2052, 2082, 2086, 2095];
  const 云墙安全超文本端口 = [443, 2053, 2083, 2087, 2096, 8443];
  const 链接列表 = [];
  const 网页套接字路径 = '/?ed=2048' + (附加查询 ? `&${附加查询}` : '');
  const 密码 = 传输路径 || 用户;
